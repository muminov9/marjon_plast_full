...
import fs from 'fs'; import path from 'path'; import {fileURLToPath} from 'url'; import PDFDocument from 'pdfkit';
import * as XLSX from 'xlsx';
const __dirname=path.dirname(fileURLToPath(import.meta.url)); const root=path.join(__dirname,'..');
const DATA=path.join(root,'data.json'); const BACKUPDIR=path.join(root,'backups');
...
const app=express(); app.disable('x-powered-by'); app.use(helmet({contentSecurityPolicy:{directives:{'script-src-attr':["'unsafe-inline'"]}}})); app.use(cors()); app.use(express.json({limit:'1mb'})); app.use(express.static(path.join(root,'public')));
...
function initial(){return {users:[],products:[],customers:[],suppliers:[],rawMaterials:[],productions:[],sales:[],purchases:[],expenses:[],workers:[],payroll:[],cash:[],settings:{companyName:'Marjon Plast',currency:'UZS',openingCash:0},audit:[]}}
function load(){
  if(!fs.existsSync(DATA)){const x=initial();fs.writeFileSync(DATA,JSON.stringify(x,null,2));return x}
  try{
    const parsed=JSON.parse(fs.readFileSync(DATA,'utf8'));
    return Object.assign(initial(),parsed,{settings:Object.assign(initial().settings,parsed.settings||{})});
  }catch(e){
    console.error('data.json buzilgan ekan, zaxira saqlab bo‘sh holatdan boshlanmoqda:',e.message);
    try{const ts=new Date().toISOString().replace(/[:.]/g,'-');fs.copyFileSync(DATA,path.join(root,'corrupt-'+ts+'.json'))}catch{}
    return initial();
  }
}
let db=load();
...
function save(){const tmp=DATA+'.tmp';fs.writeFileSync(tmp,JSON.stringify(db,null,2));fs.renameSync(tmp,DATA);backupIfDue()}
function log(user,action,entity,entityId,details=''){db.audit.unshift({id:id('log'),date:new Date().toISOString(),userId:user?.id||'',userName:user?.name||'System',action,entity,entityId,details});db.audit=db.audit.slice(0,500);save()}
...
app.post('/api/auth/bootstrap',async(req,res)=>{if(db.users.length)return res.status(409).json({message:'Admin allaqachon yaratilgan'});const {name,username,password}=req.body;if(!name||!username||!password||password.length<6)return res.status(400).json({message:'Ism, login va kamida 6 belgili parol kerak'});const u={id:id('usr'),name:String(name).trim(),username:String(username).trim(),passwordHash:await bcrypt.hash(password,12),role:'ADMIN',active:true,createdAt:new Date().toISOString()};db.users.push(u);save();log(u,'CREATE','user',u.id,u.username);res.json({user:safeUser(u)})});
...
app.post('/api/users',auth,admin,async(req,res)=>{
  ...
  db.users.push(u);save();log(req.user,'CREATE','user',u.id,u.username);res.json(safeUser(u));
});
...
app.listen(PORT,()=>console.log(`Marjon Plast: http://localhost:${PORT}`));